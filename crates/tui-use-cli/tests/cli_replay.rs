...
fn temp_dir(prefix: &str) -> PathBuf {
    ...
    let stamp = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis();
    dir.push(format!("tui-use-cli-replay-{prefix}-{stamp}"));
...
fn replay_succeeds_for_matching_snapshots() {
    ...
    let run_output = Command::new(env!("CARGO_BIN_EXE_tui-use"))
        .args([
            "run",
            "--json",
            "--scenario",
...
fn replay_detects_snapshot_mismatch() {
    ...
    let err: tui_use::model::ErrorInfo = serde_json::from_slice(&replay_output.stdout).unwrap();
...
fn replay_explain_uses_policy_defaults() {
    ...
    policy.replay.normalization_filters =
        Some(vec![tui_use::model::NormalizationFilter::SnapshotId]);
    let scenario = build_scenario(&dir, policy);
    write_scenario(&scenario_path, &scenario);
...
fn replay_report_reads_latest_summary() {
    ...
    let report_output = Command::new(env!("CARGO_BIN_EXE_tui-use"))
        .args([
            "replay-report",
            "--json",
            "--artifacts",
