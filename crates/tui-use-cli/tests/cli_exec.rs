...
fn driver_protocol_version_mismatch_is_reported() {
    ...
    assert_eq!(
        context
            ...
            .unwrap_or_default(),
        tui_use::model::PROTOCOL_VERSION as u64
    );
...
fn driver_protocol_version_ok_returns_observation() {
...

    {
        ...
        let payload = serde_json::json!({
            "protocol_version": tui_use::model::PROTOCOL_VERSION,
            "action": {
                "type": "terminate",
                "payload": {}
            }
    ...
    assert_eq!(
        ...
        tui_use::model::PROTOCOL_VERSION
    );
}
...
fn run_explain_policy_uses_scenario_config() {
    ...
    let policy = base_policy(&dir, Vec::new());
    let scenario = tui_use::model::Scenario {
        scenario_version: 1,
        metadata: tui_use::model::ScenarioMetadata {
