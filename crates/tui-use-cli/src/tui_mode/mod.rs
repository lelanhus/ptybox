...
use tui_use::model::{RunResult, Scenario, ScreenSnapshot, StepStatus};
use tui_use::runner::{run_scenario, ProgressCallback, ProgressEvent, RunnerOptions};
...
enum TuiEvent {
    ...
    RunFinished(std::result::Result<RunResult, tui_use::runner::RunnerError>),
}
...
fn render_footer(f: &mut Frame, area: Rect, app: &App) {
    let status = if app.running {
        ...
        match result.status {
            tui_use::model::RunStatus::Passed => {
