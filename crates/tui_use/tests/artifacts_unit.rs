...
fn artifacts_writer_fails_if_exists_and_no_overwrite() {
    ...
    match result {
        Err(err) => assert_eq!(err.code, "E_POLICY_DENIED"),
...
fn artifacts_writer_allows_overwrite_if_enabled() {
    ...
    let result = ArtifactsWriter::new(run_id, config);
    assert!(result.is_ok(), "Should succeed with overwrite=true: {:?}", result.err());
...
fn artifacts_write_multiple_snapshots_numbered() {
    ...
    for _ in 0..3 {
        writer.write_snapshot(&snapshot).expect("Failed to write snapshot");
    }
...
fn artifacts_write_transcript() {
    ...
    assert!(result.is_ok(), "Should write transcript: {:?}", result.err());
...
fn artifacts_checksum_correctness() {
    ...
    // Write some data
    let policy = Policy::default();
    writer.write_policy(&policy).expect("Failed to write policy");
    ...
    let checksums: serde_json::Value = serde_json::from_str(&content).expect("Should be valid JSON");
...
fn artifacts_checksum_updates_on_write() {
    ...
    writer.write_normalization(&record).expect("Failed to write normalization");

    // Read checksums after first write
    let checksums_path = dir.join("checksums.json");
    let content1 = fs::read_to_string(&checksums_path).expect("Failed to read checksums");
    let checksums1: serde_json::Value = serde_json::from_str(&content1).expect("Should be valid JSON");
    ...
    let checksums2: serde_json::Value = serde_json::from_str(&content2).expect("Should be valid JSON");
