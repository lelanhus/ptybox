...
fn temp_dir(prefix: &str) -> PathBuf {
    ...
    let stamp = SystemTime::now()
        ...
        .unwrap_or_default()
        .as_millis();
    dir.push(format!("ptybox-cli-budget-{prefix}-{stamp}"));
...
fn assert_error_with_code(output: &std::process::Output, code: &str, exit_code: i32) {
    ...
    if value.get("code").is_some() {
        let err: ptybox::model::ErrorInfo = serde_json::from_value(value).unwrap();
        assert_eq!(err.code, code);
        return;
    }
    let run: ptybox::model::RunResult = serde_json::from_value(value).unwrap();
...
fn scenario_max_steps_boundary_just_below_limit() {
    ...
    assert!(
        matches!(run.status, ptybox::model::RunStatus::Passed),
...
fn terminal_resize_boundary_exceeds_max() {
    ...
    if output.status.success() {
        let run: RunResult = serde_json::from_slice(&output.stdout).unwrap();
        assert!(
            matches!(run.status, ptybox::model::RunStatus::Failed),
