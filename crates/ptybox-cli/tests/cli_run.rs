...
fn run_scenario_outputs_passed_run_result() {
    ...
    if !matches!(run.status, ptybox::model::RunStatus::Passed) {
        panic!("run failed: {:#?}", run);
    }
    assert!(run.steps.as_ref().unwrap()[0]
        .status
        .eq(&ptybox::model::StepStatus::Passed));
}
...
fn run_scenario_supports_resize_action() {
    ...
    let run: RunResult = serde_json::from_slice(&output.stdout).unwrap();
    assert!(matches!(run.status, ptybox::model::RunStatus::Passed));
    let snapshot: ptybox::model::ScreenSnapshot = serde_json::from_str(
...
fn run_scenario_is_deterministic_for_same_inputs() {
    ...
    let output_a = Command::new(env!("CARGO_BIN_EXE_ptybox"))
        .args([
            "run",
            "--json",
            "--artifacts",
    ...
    let output_b = Command::new(env!("CARGO_BIN_EXE_ptybox"))
        .args([
            "run",
            "--json",
            "--artifacts",
    ...
    let snapshot_a: ptybox::model::ScreenSnapshot = serde_json::from_str(
        &fs::read_to_string(artifacts_a.join("snapshots/000001.json")).unwrap(),
    )
    .unwrap();
    let snapshot_b: ptybox::model::ScreenSnapshot = serde_json::from_str(
...
fn run_scenario_policy_file_writes_effective_policy() {
    ...
    let scenario = Scenario {
        ...
        run: ptybox::model::RunConfig {
            ...
            cwd: Some(dir.display().to_string()),
            initial_size: TerminalSize::default(),
            policy: ptybox::model::scenario::PolicyRef::File {
...
fn artifacts_layout_is_written() {
    ...
    let normalization: ptybox::model::NormalizationRecord = serde_json::from_str(
    ...
    assert_eq!(
        ...
        ptybox::model::NormalizationSource::None
    );
}
