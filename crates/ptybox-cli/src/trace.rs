...
fn render_html(run_result: &RunResult, snapshots: &[ScreenSnapshot], transcript: &str) -> String {
    ...
    let status_class = match run_result.status {
        ptybox::model::RunStatus::Passed => "status-passed",
        ptybox::model::RunStatus::Failed => "status-failed",
        ptybox::model::RunStatus::Errored => "status-errored",
...
<head>
    ...
    <title>ptybox Trace Viewer - {run_id}</title>
    <style>
{CSS}
    </style>
</head>
